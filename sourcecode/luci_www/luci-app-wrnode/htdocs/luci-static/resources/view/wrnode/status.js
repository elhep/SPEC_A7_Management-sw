...
return view.extend({
			...
			if (poll_wrnode_status_timeout == 0) {
				...
				.then(function(){
					...
					statusRows.forEach(function(row) {
    					var e = document.getElementById(row + '_label');
    					if (e && json[row] !== undefined)
        					e.innerHTML = String.format('%s', json[row]['label']);
						var e = document.getElementById(row + '_value');
    					if (e && json[row] !== undefined)
        					e.innerHTML = String.format('%s', json[row]['value']);
					});
				});
			}
			else {
	...
	render: function () {		
		var body = E([
			E('h2', _('WR node STATUS'))
		]);

		// ************************************************

		var title = E('h3', _('All parameters'));
		var table = E('table', { 'class': 'table' });
		statusRows.forEach(function(row) {
    		table.appendChild(E('tr', { 'class': 'tr' }, [
        		E('td', { 'id': row + '_label', 'class': 'td left', 'width': '33%' }, '-'),
        		E('td', { 'id': row + '_value', 'class': 'td left' }, '-')
    		]));
		});
